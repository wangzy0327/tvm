...
class SYCLWrappedFunc {
 public:
  ...
  void operator()(TVMArgs args, TVMRetValue* rv, void** void_args) const {
    VLOG(1) << "enter sycl wrapped func operator()";
    ...
    VLOG(0)<< "sycl  "<<func_name_<<"  grid_dim["<<wl.grid_dim(0)<<","<<wl.grid_dim(1)<<","<<wl.grid_dim(2)<<"]  grid_dim[0][1][2]";
    // sycl::range<3> k0_dimGrid(wl.grid_dim(2), wl.grid_dim(1), wl.grid_dim(0));
    ...
    VLOG(0)<< "sycl  "<<func_name_<<"  block_dim["<<wl.block_dim(2)<<","<<wl.block_dim(1)<<","<<wl.block_dim(0)<<"] block_dim[2][1][0]";
...
PackedFunc SYCLModuleNode::GetFunction(const std::string& name,
  ...
  VLOG(1) << "SYCLModuleNode::GetFunction: " << name;
  ICHECK_EQ(sptr_to_self.get(), this);
  ICHECK_NE(name, symbol::tvm_module_main) << "Device function do not have main";
  VLOG(1) << "SYCLModuleNode::find fmap_";

  auto it = fmap_.find(name);
  if (it == fmap_.end()) return PackedFunc();
  const FunctionInfo& info = it->second;
  VLOG(1) << "SYCLModuleNode::begin SYCLWrappedFunc";
  ...
  VLOG(1) << "SYCLModuleNode::begin initialize the wrapped func:";
  // initialize the wrapped func.
  f.Init(this, name, info.arg_types.size(), info.launch_param_tags, so_handler_);
  VLOG(1) << "SYCLModuleNode::finish initialize the wrapped func:";
...
void SYCLModuleNode::Init() {
  ...
  if(this->lib_compiler.load_from_file){
    ...
    VLOG(1) << this->lib_compiler.command;
